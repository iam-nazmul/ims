...
from .qt import *
from .db import db
from .widgets import DIALOG_QSS, ListDialog, DataTable, info, error, confirm

ROLES = ["Admin", "Manager", "Supervisor", "Staff"]

ROLE_DESCRIPTIONS = {
    "Admin": "Full access, including User Management and Database Restore.",
    "Manager": "Manages daily operations across all modules.",
    "Supervisor": "Oversees staff activity and approvals.",
    "Staff": "Day-to-day data entry access.",
}
...
class RolesDialog(QDialog):
    """Read-only reference: the fixed set of default roles and how many users hold each.
    Roles are assigned per-user from the Role field in User Management."""
    ...
    def __init__(self, parent=None):
        ...
        lay.addWidget(QLabel(
            "Every user account is assigned one of the following default roles\n"
            "from the Role field in User Management:"))
        table = DataTable(["Role", "Description", "Users"])
        counts = {r["role"]: r["n"] for r in
                  db().fetch_all("SELECT role, count(*) AS n FROM users GROUP BY role")}
        table.set_rows([(role, role, ROLE_DESCRIPTIONS[role], counts.get(role, 0))
                        for role in ROLES])
